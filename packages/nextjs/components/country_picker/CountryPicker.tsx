  ...
  return (
    <div ref={ref}>
      <div className="my-5 relative">
        <button
          type="button"
          className={`relative w-full border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm`}
        ...
        <AnimatePresence>
          {open && (
            <motion.ul
              ...
              className="absolute z-10 mt-1 w-full bg-[#3647A4] shadow-lg max-h-80 rounded-md text-base ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm"
              tabIndex={-1}
              role="listbox"
              aria-labelledby="listbox-label"
              aria-activedescendant="listbox-option-3"
            >
              <div className="sticky top-0 z-10 bg-[#3647A4]">
                <li className=" cursor-default select-none relative py-2 px-3">
                  <input
                    type="search"
                    name="search"
                    autoComplete={"off"}
                    className="focus:ring-blue-500 focus:border-blue-500 block w-full sm:text-sm h-10 p-4 rounded-md"
              ...
              >
                ...
                0 ? (
                  <li className=" cursor-default select-none relative py-2 pl-3 pr-9">No countries found</li>
                ) : (
                  COUNTRIES.filter(country => country.title.toLowerCase().startsWith(query.toLowerCase())).map(
                    (value, index) => {
                      return (
                        <li
                          key={`${id}-${index}`}
                          className=" cursor-default select-none relative py-2 pl-3 pr-9 flex items-center hover:bg-[#0C1B36] transition"
                          id="listbox-option-0"
                          role="option"
                          onClick={() => {
