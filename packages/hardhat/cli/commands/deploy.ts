...
export const deploy = async ({
  ...
  if (!signupGatekeeperContractAddress) {
    const contract = await deployFreeForAllSignUpGatekeeper(hre, signer, true);
    signupGatekeeperContractAddress = await contract.getAddress();
  }

  // deploy a verifier contract
  const verifierContract = await deployVerifier(hre, signer, true);

  // topup credit
  const topUpCredit = await deployTopupCredit(hre, signer, true);
